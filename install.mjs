...
function applyCodingAgent(devcontainerContent, agent) {
    if (agent === 'skip')
...
function applyPorts(content, file, aemPort) {
    if (file === 'start.sh') {
...
function applyDebug(content, file, debugPort) {
    if (file === 'start.sh') {
        return content.replace(/exec java /, `exec java -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=*:${debugPort} `);
    }
    if (file === '.devcontainer/start-aem.sh') {
        // Add debug port mapping to docker run
        return content.replace(/-p \$\{AEM_PORT\}:\$\{AEM_PORT\}/, `-p \${AEM_PORT}:\${AEM_PORT} \\\n  -p ${debugPort}:${debugPort}`);
    }
    if (file === '.devcontainer/devcontainer.json') {
        const json = JSON.parse(content);
